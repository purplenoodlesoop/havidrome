...
spec = do
  ...
  describe "listArtists" $ do
    it "hands back every artist, alphabetically" $
      answering (Right artistsAnswer) (fmap (fmap (map artistName)) . listArtists)
  ...
  describe "listAlbums" $ do
    it "hands back the albums of the artist it asked for, oldest year first" $
      answering
        (Right albumsAnswer)
        (\client -> fmap (fmap (map albumName)) (listAlbums client (ArtistId "a1")))
  ...
  describe "listSongs" $ do
    it "hands back the songs of the album it asked for, in album order" $
      answering
        (Right songsAnswer)
        (\client -> fmap (fmap (map songTitle)) (listSongs client (AlbumId "b1")))
