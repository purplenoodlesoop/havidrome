...
withSession :: (Standin -> Session -> IO a) -> IO a
withSession use = do
  standin <- newStandin
...
spec = do
  ...
  describe "picking a song" $ do
    it "plays that song, from its beginning" $ withSession $ \standin session -> do
    ...
    it "then plays the rest of its album, in album order, with nothing more asked" $
      withSession $ \standin session -> do
        start session (tracks `at` 1)
  ...
  describe "next" $ do
    it "plays the next song of the album" $ withSession $ \standin session -> do
      start session (tracks `at` 0)
      next session
      current session `shouldReturn` Just (tracks !! 1)
      loaded standin `shouldReturn` map from (take 2 tracks)

    it "ends the playing on the last song" $ withSession $ \standin session -> do
  ...
  describe "a server that cannot be reached" $ do
    it "stops the playing and shows the failure" $ withSession $ \standin session -> do
    ...
    it "starts no further song of the album" $ withSession $ \standin session -> do
      start session (tracks `at` 1)
  ...
  describe "pausing, resuming and seeking" $ do
    ...
    it "lets it run on from where it was held" $ withSession $ \standin session -> do
      start session (tracks `at` 1)
      reach standin (Seconds 60)
      pause session
      resume session
