  ...
  describe "seeking" $ do
    ...
    it "lands at the end rather than past it, and the track then finishes" $
      withTone (answering True) $ \audio track -> do
        ...
        settle
        seekBy audio 300
        reached audio `shouldReturn` Just (trackDuration track)
        resume audio
        finished <- waitForEvent audio
        finished `shouldBe` Just Finished
  ...
  describe "stopping" $
    it "plays nothing more, and does not call that finishing" $
      withTone (answering True) $ \audio track -> do
        ...
        _ <- waitUntil (fmap (>= Just (Seconds 1)) (reached audio))
        stop audio
        quiet <- timeout 1500000 (awaitEvent audio)
        quiet `shouldBe` Nothing
        nowPlaying audio `shouldReturn` Stopped
...
reached audio = fmap position (nowPlaying audio)
 where
...
waitForEvent = timeout 10000000 . awaitEvent
