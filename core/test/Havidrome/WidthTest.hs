...
tests =
  Group
    ...
    [
      ...
      , example (map char "aZ0 ~!" === [1, 1, 1, 1, 1, 1])
      )
    ,
    ...
    ,
      ...
      , example do
          map char "\x0301\x20DD\xFE0F" === [0, 0, 0]
          text "e\x0301" === 1
      )
    ,
      ( "char gives an East Asian wide or full-width character two"
      , example (map char "音\x1100\xAC00\xFF21\x20000" === [2, 2, 2, 2, 2])
      )
    ,
      ( "char gives the player's own glyphs one each"
      , example (map char "⏵⏸▶█░…⠋" === [1, 1, 1, 1, 1, 1, 1])
      )
    ,
      ( "char gives a Latin letter outside ASCII one"
      , example (map char "äöüé" === [1, 1, 1, 1])
      )
    ,
    ...
    ,
      ...
      , property do
          ...
          said <- forAll line
          assert (not (Text.any isControl (shorten room said)))
      )
    ]
