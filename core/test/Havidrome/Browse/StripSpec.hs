...
motion :: Spec
motion = describe "whether the song's audio runs or is held" $ do
...
aSkippedTrack :: Spec
aSkippedTrack = describe "a skipped track's reason" $ do
...
aNetworkFailure :: Spec
aNetworkFailure = describe "a network failure's reason" $ do
...
saidByPlayer :: Spec
saidByPlayer = describe "what the player itself has to say" $ do
  it "takes the strip over from the overlay" $
...
lastHeard :: Spec
lastHeard = describe "the last reason heard in one beat" $
