...
-- | What the strip carries, and when it carries nothing.
carrying :: Checks
carrying =
  [
...
-- | How the now-playing line is laid across the width there is.
layout :: Checks
layout =
  [
...
-- | The symbol before the track name, running or held.
symbols :: Checks
symbols =
  [
...
-- | A skipped track's reason, and the few seconds it holds the strip.
skips :: Checks
skips =
  [
...
-- | A failure of the server's, which stays until a key is pressed.
outages :: Checks
outages =
  [
...
-- | What the player itself has to say.
complaints :: Checks
complaints =
  [
