  ...
  Group
    ...
    ,
      ...
      , example do
          (loading, sounding) <- driving $ \standin session -> do
            ...
            begin standin
            (,) loading <$> session.nowPlaying
          loading === Just (Playing (tracks !! 1) (Seconds 0) Picked Loading)
          sounding === Just (Playing (tracks !! 1) (Seconds 0) Picked (Sounding Running))
      )
    ,
      ...
      , example do
          now <- driving $ \_ session -> do
            ...
            session.start (tracks `at` 2)
            session.nowPlaying
          now === Just (Playing (tracks !! 2) (Seconds 0) Picked Loading)
      )
    ,
      ...
      , example do
          now <- driving $ \standin session -> do
            ...
            session.nowPlaying
          now === Just (Playing (tracks !! 2) (Seconds 0) Followed Loading)
      )
    ,
      ...
      , example do
          (onward, back) <- driving $ \_ session -> do
            ...
            session.previous
            (,) onward <$> session.nowPlaying
          onward === Just (Playing (tracks !! 2) (Seconds 0) Followed Loading)
          back === Just (Playing (tracks !! 1) (Seconds 0) Followed Loading)
      )
    ,
    ...
    ,
      ...
      , example do
          (now, motion) <- driving $ \standin session -> do
            ...
            begin standin
            (,) <$> session.nowPlaying <*> motionOf standin
          now === Just (Playing (tracks !! 1) (Seconds 0) Picked (Sounding Paused))
          motion === Just Paused
      )
    ,
      ...
      , example do
          (song, told) <- driving $ \standin session -> do
            let other = album "b" 3
            session.start (tracks `at` 0)
            session.start (other `at` 1)
            _ <- runOut standin session 2
            (,) <$> current session <*> loaded standin
          song === Nothing
          told === from (tracks !! 0) : map from (drop 1 (album "b" 3))
      )
    ,
      ...
      , example do
          (song, told) <- driving $ \standin session -> do
            ...
            (,) <$> current session <*> loaded standin
          song === Just (tracks !! 1)
          told === map from (take 2 tracks)
      )
    ,
      ( "next ends the playing on the last song"
      , example do
          (song, motion) <- driving $ \standin session -> do
            session.start (tracks `at` 3)
            session.next
    ...
    ,
      ...
      , example do
          (song, told) <- driving $ \standin session -> do
            ...
            (,) <$> current session <*> loaded standin
          song === Just (tracks !! 1)
          told === [from (tracks !! 2), from (tracks !! 1)]
      )
    ,
      ...
      , example do
          (song, told) <- driving $ \standin session -> do
            ...
            (,) <$> current session <*> loaded standin
          song === Just (tracks !! 0)
          told === [from (tracks !! 0), from (tracks !! 0)]
      )
    ,
      ...
      , example do
          ...
          shown === [failure]
          song === Just (tracks !! 2)
          told === [from (tracks !! 1), from (tracks !! 2)]
      )
    ,
    ...
    ,
      ...
      , example do
          ...
          motion === Just Paused
          now === Just (Playing (tracks !! 1) (Seconds 60) Picked Loading)
          told === [from (tracks !! 1)]
      )
    ,
    ...
    ,
      ...
      , example do
          ...
          now === Just (Playing (tracks !! 1) (Seconds 90) Picked Loading)
          told === [from (tracks !! 1)]
      )
    ,
    ...
    ,
      ...
      , property do
          ...
          told <- driving $ \standin session -> do
            session.start (picked `at` place)
            _ <- runOut standin session (length picked - place)
            loaded standin
          told === map from (drop place picked)
      )
    ,
    ...
    ,
      ...
      , property do
          ...
          song <- driving $ \_ session -> do
            ...
            current session
          song === (if place + presses < length picked then Just (picked !! (place + presses)) else Nothing)
      )
    ,
      ...
      , property do
          ...
          song <- driving $ \_ session -> do
            ...
            current session
          song === Just (picked !! max 0 (place - presses))
      )
    ,
      ...
      , property do
          ...
          shown === [failure]
          song === (if place + 1 < length picked then Just (picked !! (place + 1)) else Nothing)
      )
    ,
