...
spec = do
  ...
  describe "how the song playing came to be playing" $ do
    it "is picked for a song the album was started from, loading until its audio starts" $
      withStandin $ \standin session -> do
        session.start (tracks `at` 1)
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 0) Picked Loading)
        begin standin
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 0) Picked (Sounding Running))

    it "is picked, and loading, for a song picked while another still loads" $
      withStandin $ \_ session -> do
        session.start (tracks `at` 1)
        session.start (tracks `at` 2)
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 2) (Seconds 0) Picked Loading)

    it "is followed for the song the album runs on to" $ withStandin $ \standin session -> do
      session.start (tracks `at` 1)
      begin standin
      _ <- runOut standin session 1
      session.nowPlaying `shouldReturn` Just (Playing (tracks !! 2) (Seconds 0) Followed Loading)
    ...
    it "is followed for the songs next and previous move to" $
      withStandin $ \_ session -> do
        ...
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 2) (Seconds 0) Followed Loading)
        session.previous
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 0) Followed Loading)
    ...
    it "leaves a picked song held while it loads held at its start once loaded" $
      withStandin $ \standin session -> do
        ...
        session.togglePause
        begin standin
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 0) Picked (Sounding Paused))
  ...
  describe "pausing, resuming and seeking" $ do
    it "holds the audio, and leaves the song where it is" $
      withStandin $ \standin session -> do
        ...
        session.pause
        motionOf standin `shouldReturn` Just Paused
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 60) Picked Loading)
        loaded standin `shouldReturn` [from (tracks !! 1)]
    ...
    it "moves through the song without changing which song it is" $
      withStandin $ \standin session -> do
        ...
        reach standin (Seconds 60)
        session.seekBy 30
        session.nowPlaying `shouldReturn` Just (Playing (tracks !! 1) (Seconds 90) Picked Loading)
        loaded standin `shouldReturn` [from (tracks !! 1)]
